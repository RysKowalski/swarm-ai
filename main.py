...
def train_ai(map: Map, model, replay_buffer, batch_size=64, epsilon=0.1, gamma=0.99, learning_rate=0.001, num_generations=1000):
    ...
    for generation in range(initial_experience_generations):
        ...
        for character_id in range(map.max_characters):
            ...
            char_data = map.find_character_by_id(character_id)[3:]  # Wybierz wszystkie dane
            ...
            next_state = torch.tensor(np.concatenate([map.get_surroundings(character_id).flatten(), map.find_character_by_id(character_id)[3:]]), dtype=torch.float32)
...
def main():
    # Inicjalizacja mapy
    map = Map(15, 15, 50, 0)  # Mapa 2000x2000, 50 postaci, dane postaci o długości 10

    # Tworzymy model
    model = SwarmAI()

    # Zapytaj użytkownika o wczytanie modelu
    load_model = input("Czy wczytać model z pliku? (tak/nie): ").strip().lower()
    if load_model == 'tak':
        try:
            model.load_state_dict(torch.load(input('podaj nazwę pliku z modelem: ')))
            print("Model wczytany poprawnie.")
        except FileNotFoundError:
            print("Nie znaleziono pliku modelu. Tworzenie nowego modelu.")

    # Zapytaj użytkownika o liczbę generacji
    try:
        num_generations = int(input("Podaj liczbę generacji do wykonania: "))
    except ValueError:
        print("Podano nieprawidłową wartość. Domyślna liczba generacji: 1000.")
    ...
    # Trenujemy model
    train_ai(map, model, replay_buffer, num_generations=num_generations)
